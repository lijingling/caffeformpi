...
cv::Mat addNoiseToImage(cv::Mat src) {
  cv::Point2f srcTri[3];
  cv::Point2f dstTri[3];
  ...
  srcTri[0] = cv::Point2f( 38, 55 );
  srcTri[1] = cv::Point2f( 89, 55 );
  srcTri[2] = cv::Point2f( 64, 105 );

  srand( (unsigned)time(NULL));
  int rnd = rand()%729;

  dstTri[0].x = srcTri[0].x + rnd/243-1;
  dstTri[0].y = srcTri[0].y + rnd%243/81-1;
  dstTri[1].x = srcTri[1].x + rnd%81/27-1;
  dstTri[1].y = srcTri[1].y + rnd%27/9-1;
  dstTri[2].x = srcTri[2].x + rnd%9/3-1;
  dstTri[2].y = srcTri[2].y + rnd%3-1;
  
  /// Get the Affine Transform
  warp_mat = getAffineTransform( srcTri, dstTri );

  int apply_Interpolation = rand()%4;

  /// Apply the Affine Transform just found to the src image
  warpAffine(src, warp_dst, warp_mat, warp_dst.size(),apply_Interpolation);
...
void getIntensityChange(cv::Mat OriImage,cv::Mat& DstImage)//index==0:darken; index==1:brighten
{

  //LOG(INFO) << "getIntensityChange";
  double degree;//¿ÉÐÞ¸Ä

  caffe_rng_uniform(1,0.2,0.7,&degree);

  vector<cv::Mat> src(1);
  vector<cv::Mat> dst(1);
  cv::split(OriImage,src);
  cv::Mat ImageL = src[0];
  cv::Scalar meanL =  mean(ImageL);

  int index = rand()%2;        //aÎªËæ»úÊý³ýÒÔ2È¡Óà£¬ÔòÎª0»ò1

  if (index == 0)//±ä°µ
  {
    ImageL = ImageL - degree*meanL[0];
    max(ImageL,0.0,ImageL);
  }
  else if (index == 1)//±äÁÁ
  {
    ImageL = ImageL + degree*meanL[0];
    min(ImageL,255.0,ImageL);
  }

  src[0] = ImageL;
  cv::merge(src,DstImage);
}

void getContrastEnhance(cv::Mat OriImage,cv::Mat& DstImage)
{
  //LOG(INFO) << "getContrastEnhance";
  DstImage = OriImage.clone();
  double startRange;  
  double endRange;

  caffe_rng_uniform(1,0.2,0.4,&startRange);
  caffe_rng_uniform(1,0.6,0.8,&endRange);

  float alfa = 1.0 /(endRange-startRange);

  subtract(DstImage,cv::Scalar::all(startRange*255),DstImage);
  multiply(DstImage,cv::Mat(DstImage.rows,DstImage.cols,DstImage.type(),cv::Scalar::all(alfa)),DstImage);
  min(DstImage,255.0,DstImage);
  max(DstImage,0.0,DstImage);
}


cv::Mat ReadDataAugToCVMat(const string& filename, const int height, const int width, const bool is_color) 
{
  cv::Mat cv_img;
  ...
  int smooth_param1 = 3;
    ...
    int smooth_type = rand()%10; // see opencv_util.hpp
    smooth_param1 = 3 + 2*(rand()%2);
    cv::RNG rng;
    // float min_alpha = 0.8, max_alpha = 1.2;
    // alpha = rng.uniform(min_alpha, max_alpha);

    int ksize = 2 * (rand()%5) + 5; // see opencv_util.hpp
    int randnum = rand()%20;  
    double sigma = randnum/10.0;  

    switch(smooth_type){
      case 0:
        cv::GaussianBlur(cv_img, cv_img, cv::Size(ksize,ksize),sigma);
        break;
      case 1:
      ...
      case 3:
        ...
        break;
      case 4:
        cv_img = addNoiseToImage(cv_img); 
        break;
      case 5:
        getIntensityChange(cv_img, cv_img);
        break;
      case 6:
        getContrastEnhance(cv_img, cv_img);
        break;
      default: 
        cv_img = cv_img;   
      }
