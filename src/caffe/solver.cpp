...
void Solver<Dtype>::Init(const SolverParameter& param) {
  ...
  unsigned int seed = 
    param_.force_set_random_seed() && param_.random_seed() >= 0 ? 
    param_.random_seed() : time(NULL);
#ifdef USE_MPI
  if(Caffe::parallel_mode() == Caffe::MPI) {
    MPI_CHECK(MPI_Bcast(&seed, 1, MPI_UNSIGNED, 0, MPI_COMM_WORLD));
  }
#endif
  Caffe::set_random_seed(seed);
  LOG(INFO) << "Use random seed " << seed;
...
#ifdef USE_MPI
  if(Caffe::parallel_mode() == Caffe::MPI) {
    NcclBatchComm::SetCommMode(param_.comm_mode());
    NcclBatchComm::SetCommBatchSize(param_.comm_batch_size());
  }
#endif
}
...
void Solver<Dtype>::Step(int iters) {
  ...
  int nsample_per_iter = net_->top_vecs()[0][0]->shape(0);
  #ifdef USE_MPI
    if (Caffe::parallel_mode()==Caffe::MPI){
      nsample_per_iter *= Caffe::MPI_all_rank();
    }
  #endif
  ...
  while (iter_ < stop_iter) {
    ...
    if (display) {
      ...
      // log speed
      int num_iter = (iter_ - start_iter) / param_.display() == 0 ? 
        ((iter_ - start_iter) == 0 ? 1 : (iter_ - start_iter) % param_.display()) : 
        param_.display();
      float elapse = timer.MilliSeconds();
      LOG(INFO) << "    lr = " << lr << std::fixed << std::setprecision(1) <<
          ", speed = " << 
          elapse / num_iter << "ms/iter, " <<
          "throughput = " <<
          1000.0f * nsample_per_iter * num_iter / elapse << "sample/s";
      timer.Start();
    }
...
void Solver<Dtype>::SyncGradient(){
  ...
  for (int param_id = 0; param_id < net_params.size(); ++param_id) {
    ...
    bool need_data_parallel = layer->need_sync();
    bool need_model_parallel = layer->need_model_parallel();

    // conduct gradient synchronization here
    if (is_self && need_data_parallel && !need_model_parallel){
  ...
  //DLOG(INFO) << "Sacle sync gradient cost " << ter.elapsed() * 1000 << "ms"; 
}
...
void Solver<Dtype>::SyncData(){
  ...
  for (int param_id = 0; param_id < net_params.size(); ++param_id) {
    ...
    // if not, this blob has already been synced.
    bool is_self = (param_owner == -1);
    // model_parallel dose not need to sync.
    int layer_id = this->net_->param_layer_indices()[param_id].first;
    bool model_parallel = this->net_->layers()[layer_id]->need_model_parallel();
    // conduct data synchronization here
    if (is_self && !model_parallel){
      Dtype* data = net_params[param_id]->mutable_gpu_data();
      caffe_nccl_ibcast(data, net_params[param_id]->count());
    }
  }
  nccl_force_synchronize();
  LOG(INFO)<<"Model Synchronization Communication time "<< timer.MilliSeconds() <<"ms";
}
...
void Solver<Dtype>::SyncOutput(shared_ptr<Net<Dtype> > net){
  ...
  for (int j = 0; j < result.size(); ++j) {
    caffe_nccl_iallreduce<Dtype>(result[j]->mutable_gpu_data(),
      result[j]->mutable_gpu_data(), result[j]->count()); 
  }
  nccl_force_synchronize();
  for( int j = 0; j < result.size(); ++j){
...
Dtype Solver<Dtype>::SyncLoss(Dtype loss){
  loss_.mutable_cpu_data()[0] = loss;
  caffe_nccl_iallreduce<Dtype>(loss_.mutable_gpu_data(), loss_.mutable_gpu_data(), 1);
  nccl_force_synchronize();
  return loss_.cpu_data()[0] / Caffe::MPI_all_rank();
}
#endif
...
void Solver<Dtype>::Snapshot() {
  ...
  LOG(INFO) << "Snapshot using " << timer.MilliSeconds() << "ms";
}
...
template <typename Dtype>
Dtype SGDSolver<Dtype>::ApplyUpdate() {
