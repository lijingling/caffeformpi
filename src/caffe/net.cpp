...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int layer_id = 0; layer_id < param.layer_size(); ++layer_id) {
    ...
    // figure out if the layer is data-parallel
    ...
    // figure out if the layer is model-parallel
    bool need_model_parallel = layer_param.need_model_parallel();
    bool need_data_parallel = layers_[layer_id]->need_sync();
    layers_[layer_id]->set_need_model_parallel(need_model_parallel);
    // layer should be either data-parallel or model-parallel
    CHECK(!(need_model_parallel && need_data_parallel)) << 
      "layer " << layer_param.name() <<
      " should be either data-parallel or model-parallel";
    #else
    for (int bottom_id = 0; bottom_id < layer_param.bottom_size();
         ++bottom_id) {
      const int blob_id = AppendBottom(param, layer_id, bottom_id,
    ...
    #ifdef USE_MPI
    // set the diff memory to memory pool
    // for more efficient batch communication
    if(Caffe::parallel_mode() == Caffe::MPI && 
       phase_ == TRAIN && 
       layers_[layer_id]->need_sync()) {
      const vector<shared_ptr<Blob<Dtype> > > blobs = layers_[layer_id]->blobs();
      for(int blob_idx = 0; blob_idx < blobs.size(); blob_idx++){
        blobs[blob_idx]->SetDiffAllocatorType(EPoolAllocator);
        MemPool::add(blobs[blob_idx]->count() * sizeof(Dtype));
      }
    }
    #endif

    // set the top data and top diff memory to unified memory
    // for using both device and host memory
    if(layer_param.use_unify_mem_size() > 0) {
      CHECK_EQ(layer_param.use_unify_mem_size(), top_vecs_[layer_id].size())
        << "unify memory flag need to be with the same size as top vec";
    }
    for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
      if(layer_param.use_unify_mem_size() > 0 &&
         layer_param.use_unify_mem(top_id)) {
        top_vecs_[layer_id][top_id]->SetDataAllocatorType(EUnifyAllocator);
        top_vecs_[layer_id][top_id]->SetDiffAllocatorType(EUnifyAllocator);
      }
    }
  ...
  #ifdef USE_MPI
  if(Caffe::parallel_mode() == Caffe::MPI && phase_ == TRAIN) {
    MemPool::make();
  }
  #endif
...
void Net<Dtype>::BackwardFromTo(int start, int end) {
  ...
  for (int i = start; i >= end; --i) {
    ...
    if (layer_need_backward_[i]) {
      ...
      //sync gradient
      if ((Caffe::parallel_mode() == Caffe::MPI) && 
          (Caffe::remaining_sub_iter() == 0)) {
        Dtype * layer_blob_ptr = NULL;
        int layer_blob_count = 0;
        for (int n = 0; n < param_layer_indices_.size(); ++n) {
          if (param_layer_indices_[n].first == i && layers_[i]->need_sync()) {
            Dtype * data_diff = this->params_[n]->mutable_gpu_diff();
            layer_blob_ptr = layer_blob_ptr ? std::min(data_diff, layer_blob_ptr) :
                             data_diff;
            layer_blob_count += this->params_[n]->count();
          }
        }
        if(layer_blob_ptr && layer_blob_count > 0) {
          caffe_nccl_batch_iallreduce(
            layer_blob_ptr,
            layer_blob_ptr,
            layer_blob_count
          );
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < num_source_layers; ++i) {
    ...
    vector<shared_ptr<Blob<Dtype> > >& target_blobs =
        layers_[target_layer_id]->blobs();
    #ifdef USE_MPI
      bool need_model_parallel_ = 
        layers_[target_layer_id]->need_model_parallel();
      if(need_model_parallel_ &&
         Caffe::parallel_mode() == Caffe::MPI) {
        CHECK_EQ(layers_[target_layer_id]->dist_blob_counts().size(),
            target_blobs.size());
        const int my_rank = Caffe::MPI_my_rank();
        for (int j = 0; j < target_blobs.size(); ++j) {
          // figure out the offset in distributed or single model
          CHECK_EQ(layers_[target_layer_id]->dist_blob_counts()[j].size(),
            Caffe::MPI_all_rank());
          int offset = 0;
          for(int k = 0; k < my_rank; ++k) {
            offset += 
              layers_[target_layer_id]->dist_blob_counts()[j][k];
          }
          // load model from source
          if(source_layer.blobs_size() == target_blobs.size()) {
            // params are in single model
            target_blobs[j]->FromProto(source_layer.blobs(j), offset);
          }
          else if(source_layer.dist_blobs_size() == target_blobs.size()) {
            // params are in distributed model
            target_blobs[j]->FromProto(source_layer.dist_blobs(j), offset);
          }
          else {
            LOG(FATAL) << "Incompatible number of blobs for layer " << source_layer_name;
          }
        }
      }
      else {
        ...
        for (int j = 0; j < target_blobs.size(); ++j) {
          if(source_layer.blobs_size() == target_blobs.size()) {
            ...
            target_blobs[j]->FromProto(source_layer.dist_blobs(j), kReshape);
          }
          else {
            LOG(FATAL) << "Incompatible number of blobs for layer " << source_layer_name;
          }
...
template <typename Dtype>
void Net<Dtype>::ZeroParamsDiff() {
  for (int i = 0; i < params_.size(); ++i) {
    shared_ptr<Blob<Dtype> > blob = params_[i];
    switch (Caffe::mode()) {
    case Caffe::CPU:
...
void Net<Dtype>::MemoryOptimize_v2(){
  ...
  for(int i = 0; i < params_.size(); ++i) {
    count_param += params_[i]->count() * sizeof(Dtype) * 2;
  }

  LOG(INFO) << std::fixed << std::setprecision(1) 
            << "raw memory " << (count_raw + count_param) / 1024.0 / 1024 << "MiB, "
            << "opt memory " << (count_opt + count_param) / 1024.0 / 1024 << "MiB";
}
