...
void ScatterLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    ...
    caffe_mpi_scatter((Dtype*)bottom[i]->cpu_data(),top[i]->mutable_cpu_data(), top[i]->count());
  }
#else
#endif
}
...
void ScatterLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i = 0; i < bottom.size(); ++i) {
    caffe_mpi_allgather((Dtype*)top[i]->cpu_diff(),bottom[i]->mutable_cpu_diff(), top[i]->count());
