...
void DistanceLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {  
  const int num_class_per_batch = this->layer_param_.distance_loss_param().num_class_per_batch(); 
  num_class_per_batch_ = num_class_per_batch;
  num_class_total_ = num_class_per_batch * this->layer_param_.distance_loss_param().num_gpu(); 
  const int num_img_per_class = this->layer_param_.distance_loss_param().num_img_per_class(); 
  num_img_per_class_ = num_img_per_class;
  const int num_output = this->layer_param_.distance_loss_param().num_output();  
  N_ = num_output;
  const int axis = bottom[0]->CanonicalAxisIndex(
      this->layer_param_.distance_loss_param().axis());
  ...
  if (this->blobs_.size() > 0) {
    ...
    shared_ptr<Filler<Dtype> > center_filler(GetFiller<Dtype>(
        this->layer_param_.distance_loss_param().center_filler()));
...
template <typename Dtype>
void DistanceLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  CHECK_EQ(bottom[1]->channels(), 1);
  CHECK_EQ(bottom[1]->height(), 1);
  CHECK_EQ(bottom[1]->width(), 1);
  M_ = bottom[0]->num();
  CHECK_EQ(M_, num_img_per_class_ * num_class_total_);
  // The top shape will be the bottom shape with the flattened axes dropped,
  // and replaced by a single axis with dimension num_output (N_).
  LossLayer<Dtype>::Reshape(bottom, top);
  //vector<int> variation_shape = bottom[0]->shape();
  //variation_shape[0] = M_ * num_class_total_;
  //variation_.Reshape(variation_shape);
  distance_.Reshape(M_, num_class_total_, 1, 1);
  variation_sum_.ReshapeLike(*this->blobs_[0]);
  scale_.Reshape(M_, 1, 1, 1);
  //prob_.ReshapeLike(distance_);
}
...
REGISTER_LAYER_CLASS(DistanceLoss);
