...
void SoftmaxLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  if(temprature_ != Dtype(1)) {
    ...
    kernel_scale<Dtype><<<CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS>>>(
        count, t_reciprocal, top_data, top_data);
  }
  // exponentiate
  // NOLINT_NEXT_LINE(whitespace/operators)
...
void SoftmaxLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if(temprature_ != Dtype(1)) {
    Dtype t_reciprocal = Dtype(1) / temprature_;
    caffe_gpu_scal(bottom[0]->count(), t_reciprocal, bottom_diff);
  }
}
