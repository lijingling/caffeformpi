...
void CenterLossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
    ...
    std::ifstream fin1;
    fin1.open("/home/chenxiong/list/MSRA_MID_CENTER.txt");

    CHECK(fin1.is_open())<< "Could not load center !";
    std::string buffer;
    float szBuf;
    int k = 0;
    while (getline(fin1, buffer))
    {
      ...
      fin2.open(buffer.c_str(), std::ios::binary);
      for (int i = 0; i < K_; ++i)
      {
        fin2.read((char*)&szBuf, sizeof(float));
        this->blobs_[0]->mutable_cpu_data()[k * K_ + i] = szBuf;
        //std::cout<<buffer<<" "<<k<<" "<<szBuf<<std::endl;
      }
      fin2.close();
      ++k;
    }    
...
void CenterLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  E2_.Reshape(N_, 1, 1, 1); 
  distance_sq_.Reshape(M_, N_, 1, 1);
  bottom_dis_sq_.Reshape(M_, 1, 1, 1);
  center_dis_sq_.Reshape(N_, 1, 1, 1);

  vector<int> block_shape(2);
  block_shape[0] = M_;
  block_shape[1] =  (N_ / COMPATIBLE_CUDA_NUM_THREADS) + 1;
