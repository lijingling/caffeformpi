...
void CuDNNSoftmaxLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  int algorithm = this->layer_param_.softmax_param().algorithm();
  switch (algorithm) {
    case caffe::SoftmaxParameter_Algorithm_FAST:
      algorithm_ = CUDNN_SOFTMAX_FAST;
      break;
    case caffe::SoftmaxParameter_Algorithm_ACCURATE:
      algorithm_ = CUDNN_SOFTMAX_ACCURATE;
      break;
    default:
      LOG(FATAL) << "unrecognized softmax algorithm " << algorithm;
  }
  // Initialize CUDNN.
...
void CuDNNSoftmaxLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if(this->temprature_ != Dtype(1)) {
    tempratured_bottom_.ReshapeLike(*bottom[0]);
  }
