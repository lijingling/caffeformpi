...
protected:
    ...
    virtual void Backward_gpu(const vector<Blob<Dtype>*>& top,
               const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom);
    Blob<Dtype> diff_; // A cache for pairwise diffs, typical size: (40xbatchsize) x 128 x 1 x 1
    Blob<Dtype> diff_sq_; // squared diff, for gpu version.
    Blob<Dtype> dist_sq_; // pairwise squared distance matrix


    //Dtype *diff_data;
    
    //Blob<Dtype> summer_vec_; // tmp storage for gpu forward pass
    //Blob<Dtype> sign_;    // margin violation sign.
};
...
template <typename Dtype>
class HierarchicalSoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:
  explicit HierarchicalSoftmaxWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param), huffman_tree_(NULL) {}
  virtual ~HierarchicalSoftmaxWithLossLayer();
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);

  virtual inline const char* type() const { return "HierarchicalSoftmaxWithLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return 1; }

 protected:
  // Huffman tree typedef
  ...
  void BuildhHuffmanTree(const std::string& frequency_hist_fn);
 ...
 protected:
  ...
  void SigmoidProduct_cpu(const Blob<Dtype>* bottom, int data_idx, int label);
  void SigmoidProduct_gpu(const Blob<Dtype>* bottom, int data_idx, int label);

  Dtype Probability_cpu(int data_idx, int label);
  Dtype Probability_gpu(int data_idx, int label);

  void ResidualAndGrad_cpu(Blob<Dtype>* bottom, int data_idx, int label);
  void ResidualAndGrad_gpu(Blob<Dtype>* bottom, int data_idx, int label);

  int num_input_;
  int num_output_;
  int batch_size_;
  huffman_node_t* huffman_tree_;
  Blob<Dtype> hidden_layer_;
  Blob<Dtype> sigmoid_product_;
};


template <typename Dtype>
class InnerProductSoftmaxWithLossLayer : public LossLayer<Dtype> {
 public:
  explicit InnerProductSoftmaxWithLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "InnerProductSoftmaxWithLoss"; }
 ...
 protected:
  ...
  shared_ptr<Layer<Dtype> > inner_product_layer_;
  shared_ptr<Layer<Dtype> > hard_softmax_layer_;
  shared_ptr<Layer<Dtype> > soft_softmax_layer_;
  Blob<Dtype> inner_product_bottom_;
  Blob<Dtype> inner_product_top_;
  Blob<Dtype> hard_softmax_top_;
  Blob<Dtype> soft_softmax_top_;
  
  vector<Blob<Dtype>*> inner_product_bottom_vec_;
  vector<Blob<Dtype>*> inner_product_top_vec_;
  vector<Blob<Dtype>*> hard_softmax_bottom_vec_;
  vector<Blob<Dtype>*> hard_softmax_top_vec_;
  vector<Blob<Dtype>*> soft_softmax_bottom_vec_;
  vector<Blob<Dtype>*> soft_softmax_top_vec_;

  int num_input_;
  int num_output_;

  int hard_bottom_idx_;
  int soft_bottom_idx_;
  vector<Dtype> tempratures_;
  ...
  Blob<Dtype> sparse_prob_sum_multiplier_;

  int sub_batch_size_;
};

template <typename Dtype>
class SnpcLossLayer : public LossLayer<Dtype> {
public:
  explicit SnpcLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
		  const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "SnpcLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
protected:
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int sub_class_;
  Blob<Dtype> E1_;   //M_ * 1
  Blob<Dtype> E2_;   //sub_class_ * 1
  Blob<Dtype> E3_;   //K_ * 1
  Blob<Dtype> row_sum_; //M_ * 1
  Blob<Dtype> col_sum_; //sub_class_ * 1
  Blob<Dtype> distance_sq_;  //M_ * sub_class_
  Blob<Dtype> bottom_dis_sq_; //M_ * 1
  Blob<Dtype> center_dis_sq_; //sub_class_ * 1

  Blob<Dtype> sum_multiplier_; //sub_class_ * K_
  Blob<Dtype> block_;
};
...
class DistanceLossLayer : public LossLayer<Dtype> {
 public:
  ...
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "DistanceLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
 protected:
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  int K_;
  int N_;
  int num_class_per_batch_;
  int num_img_per_class_;
  int num_class_total_;
  Blob<Dtype> distance_;
  //Blob<Dtype> variation_;
  Blob<Dtype> variation_sum_;
  Blob<Dtype> scale_;
  //Blob<Dtype> repeat_labels_
  //Blob<Dtype> prob_;
};
...
template <typename Dtype>
class LvqLossLayer : public LossLayer<Dtype> {
public:
  explicit LvqLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual inline const char* type() const { return "LvqLoss"; }
  virtual inline int ExactNumBottomBlobs() const { return 2; }
  virtual inline int ExactNumTopBlobs() const { return -1; }
protected:
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
