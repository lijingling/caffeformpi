 ...
 */
template <typename Dtype>
class RegressionDataLayer : public BasePrefetchingDataLayer<Dtype> {
public:
	explicit RegressionDataLayer(const LayerParameter& param)
	: BasePrefetchingDataLayer<Dtype>(param) {}
	virtual ~RegressionDataLayer();
	virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);

	virtual inline const char* type() const { return "RegressionData"; }
	virtual inline int ExactNumBottomBlobs() const { return 0; }
...
protected:
	...
	int lines_id_;
	// virtual void split(const string& src, const string& separator, vector<string>& dest);
};
/**
 * @brief Provides data to the Net from image files.
 *
 ...
 */
template <typename Dtype>
class TriRegDataLayer : public BasePrefetchingDataLayer<Dtype> {
public:
	explicit TriRegDataLayer(const LayerParameter& param)
	: BasePrefetchingDataLayer<Dtype>(param) {}
	virtual ~TriRegDataLayer();
	virtual void DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
			const vector<Blob<Dtype>*>& top);

	virtual inline const char* type() const { return "TriRegData"; }
	virtual inline int ExactNumBottomBlobs() const { return 0; }
	virtual inline int ExactNumTopBlobs() const { return 2; }
...
protected:
	...
	inline virtual void advance_cursor(){
		...
		if (lines_id_ >= people_size) {
			...
			if (this->layer_param_.trireg_data_param().shuffle()) {
				ShuffleImages();
			}
		}
	...
	int lines_id_;
	int fea_dim;
	int negative_lines_id_;
	std::map<int, vector<std::string> > labels_filenames;

	vector<std::pair<std::string, int> > img_lines_negative;
	...
	vector<vector<std::pair<std::string, int> > >img_lines;
	vector<std::pair<std::string, int> > neg_img_lines;
	vector<vector<std::vector<int> > >img_idx ;

	std::vector<string> vNegSample_;
};
...
protected:
	...
	inline virtual void advance_cursor(){
		...
		if (lines_id_ >= people_size) {
			...
			lines_id_ = 0;
			// std::random_shuffle(rand_idx.begin(), rand_idx.end());
			if (this->layer_param_.triplet_data_param().shuffle()) {
				ShuffleImages();
			}
		}
		if(negative_lines_id_ >= img_lines_negative.size()){
