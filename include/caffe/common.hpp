...
#define NCCL_CHECK(cond) \
do { \
    int status = cond; \
    CHECK_EQ(status, ncclSuccess) << " " \
      << "NCCL Error Code: " << status; \
  } while (0)
...
void SetLogDir(const char* program_name, const char* log_dir);
...
class Caffe {
 public:
  ...
  inline static void MPI_build_rank(){
    MPI_CHECK(MPI_Comm_rank(MPI_COMM_WORLD, &(Get().mpi_my_rank_)));
    MPI_CHECK(MPI_Comm_size(MPI_COMM_WORLD, &(Get().mpi_all_rank_)));
  }
  inline static ncclComm_t NCCL_COMM() {return Get().nccl_comm_;}
  inline static void NCCL_init() {
    ncclUniqueId commId;
    NCCL_CHECK(ncclGetUniqueId(&commId));
    MPI_CHECK(MPI_Bcast(&commId, NCCL_UNIQUE_ID_BYTES, MPI_CHAR, 0, MPI_COMM_WORLD));
    NCCL_CHECK(ncclCommInitRank(&(Get().nccl_comm_), 
            Get().mpi_all_rank_, commId, Get().mpi_my_rank_));
  }
  inline static void Finalize() {ncclCommDestroy(Get().nccl_comm_);}
