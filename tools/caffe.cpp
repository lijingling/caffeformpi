...
int train() {
  ...
  if (Caffe::parallel_mode() == Caffe::MPI) {
    CHECK_EQ(Caffe::mode(), Caffe::GPU) <<
      "It is not allow to train with CPU in MPI mode";
  }
#endif
...
int time() {
  ...
  boost::shared_ptr<Timer> iter_timer;
  boost::shared_ptr<Timer> total_timer;
  boost::shared_ptr<Timer> forward_timer;
  boost::shared_ptr<Timer> backward_timer;
  if (Caffe::mode() == Caffe::GPU) {
    #ifndef CPU_ONLY
    timer.reset(new GPUTimer());
    iter_timer.reset(new GPUTimer());
    total_timer.reset(new GPUTimer());
    forward_timer.reset(new GPUTimer());
    backward_timer.reset(new GPUTimer());
    #else
    NO_GPU;
    #endif
  }
  else {
    timer.reset(new CPUTimer());
    iter_timer.reset(new CPUTimer());
    total_timer.reset(new CPUTimer());
    forward_timer.reset(new CPUTimer());
    backward_timer.reset(new CPUTimer());
  }
